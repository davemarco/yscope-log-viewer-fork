...
import { isAutoGeneratedKey } from "../../../utils/formatters";
import { parseFilterKey } from "../../../utils/decoders";
...
class JsonlDecoder implements Decoder {
    ...
    constructor (dataArray: Uint8Array, decoderOptions: DecoderOptions) {
        ...
        if (parsedLogLevelKey.isAutoGenerated || parsedTimestampKey.isAutoGenerated) {
            throw new Error(
                "`@` is a reserved symbol in the format string and must be escaped with `\\` " +
                "for user-generated keys."
            );
        }
    ...
    #parseJson (line: string) {
        ...
        try {
            ...
            level = convertToLogLevelValue(getNestedJsonValue(fields,this.#logLevelSplitKeys));
            timestamp = convertToDayjsTimestamp(getNestedJsonValue(fields,this.#timestampSplitKey));
        } catch (e) {
            if (0 === line.length) {
                return;
            }
