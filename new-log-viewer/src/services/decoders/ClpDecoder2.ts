...
/**
 * List of byte sizes for each segment in archive. Index corresponds to segment number. Sizes are
 * used to locate and decode file in byte stream.
 */
type SegmentFileSizes = number[];

/**
 * Byte sizes for non-segment archive files. Sizes are used to locate and decode file in byte stream.
 */
...
interface SegmentInfo {
  numMessages: number;
  numVariables: number;
  // Byte offsets in segment.
  segmentTimestampsPosition: number;
  segmentLogTypesPosition: number;
  segmentVariablesPosition: number;
}
...
type EncodedVariableType = bigint;
...
interface LogEvent {
  ...
  logType: Uint8Array;
  dictVars: Uint8Array[];
  encodedVars: EncodedVariableType[];
}

/**
 * Minimum version of CLP single file archive supported by this decoder. If the archive header
 * contains a lower version, an error will be thrown.
 */
const minSupportedVersion: string = "0.1.0";

/**
 * A decoder for CLP single file archives. Single file archives are an alternate archive format
 * where CLP compressed output is stored in a single file rather than in a directory with multiple
 * files.
 */
class ClpArchiveDecoder implements Decoder {
  ...
  /**
   * Private constructor for ClpArchiveDecoder. This is not intended to be invoked publicly.
   * Instead, use ClpArchiveDecoder.create() to create a new instance of the class.
   *
   * @param dataInputStream Byte stream containing segment data (metadata already parsed).
  ...
  /**
   * Creates a new ClpArchiveDecoder. Single file archive metadata as well as CLP archive metadata
   * is parsed in this method. In addition, the log type dictionary and the variable dictionary
   * are decoded. The returned decoder is ready to parse segment data.
   *
   * @param dataArray Byte array containing single file archive. When this method is finished,
   * the position of the data array will be the start of segment data.
  ...
  static async create(dataArray: Uint8Array): Promise<ClpArchiveDecoder> {
    let dataInputStream: DataInputStream = new DataInputStream(
      dataArray.buffer,
      true,
    );

    const singleFileArchiveMetadata: any =
    ...
    let clpArchiveDecoder: ClpArchiveDecoder = new ClpArchiveDecoder(
      dataInputStream,
      segmentSizes,
      segmentInfos,
      logTypeDict,
  ...
  decode(beginIdx: number, endIdx: number): Nullable<DecodeResultType[]> {
    ...
    for (let logEventIdx = beginIdx; logEventIdx < endIdx; logEventIdx++) {
      let logEvent: LogEvent = this.#logEvents[logEventIdx] as LogEvent;
      const message: string = this.#messageFromLogEvent(logEvent, textDecoder);

      let logLevel: LOG_LEVEL = logEvent.logLevel;
      let timestamp: bigint = logEvent.timestamp;
  ...
  /**
   * Parses single archive preamble from data input stream and retrieves metadata.
   * The metadata is parsed by parseSingleFileArchiveMetadata.
   *
   * @param dataArray Byte stream containing single file archive.
  ...
  static #parseSingleFileArchiveMetadata(
    ...
    let nonSegmentSizes: NonSegmentFileSizes = {
      metadataDB: 0,
      logTypeDict: 0,
      logTypeSegIndex: 0,
      varDict: 0,
    ...
    let segmentSizes: SegmentFileSizes = [];

    for (let i = 0; i < fileInfos.length - 1; i++) {
      const fileInfo: any = fileInfos[i];
      const nextFileInfo: any = fileInfos[i + 1];
  ...
  static async #getSegmentInfos(
    ...
    const segmentInfos: SegmentInfo[] = queryResult[0].values.map(
      (row: string | number[]) => {
        const [
          numMessages,
          numVariables,
  ...
  static async #parseDictionary(
    ...
    let length = decompressedBytes.byteLength;
   ...
   * log types and finally variables. Number of timestamps, log types, and variables are known since already queried
  ...
  #segmentToLogEvents(segment: Segment) {
    ...
    let variablesIterator: Iterator<bigint> =
    ...
    for (let i = 0; i < numMessages; i++) {
      ...
      let logType: Uint8Array | undefined = this.#logTypeDict[logTypeIdx];
      if (!logType) {
  ...
  #messageFromLogEvent(logEvent: LogEvent, textDecoder: TextDecoder): string {
    ...
    let encodedVarsIterator: Iterator<bigint> =
      logEvent.encodedVars[Symbol.iterator]();
    let dictVarsIterator: Iterator<Uint8Array> =
